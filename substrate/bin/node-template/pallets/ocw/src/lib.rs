...
pub mod pallet {
	...
	const HTTP_ASSET_REMOTE_REQUEST: &str = "https://api.coincap.io/v2/assets/polkadot";
	// const HTTP_ASSET_REMOTE_REQUEST: &str = "http://127.0.0.1:8000/j.json";
	...
	#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug)]
	pub struct Price<Public> {
		price: (u64, Permill),
		public: Public,
	}

	impl<T: SigningTypes> SignedPayload<T> for Price<T::Public> {
		fn public(&self) -> T::Public {
			self.public.clone()
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_dot_price_with_signed_payload(
			origin: OriginFor<T>,
			price: Price<T::Public>,
			_signature: T::Signature,
		) -> DispatchResult {
			let _ = ensure_none(origin)?;
			let Price { price, public } = price;
			log::info!("submit_dot_price_with_signed_payload: ({:?}, {:?})", price, public);
	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			// 你们自己选一种方法提交回链上，并在代码注释为什么用这种方法提交回链上最好。
			// 只保留当前最近的 10 个价格， 其他价格可丢弃 （就是 Vec 的长度长到 10
			// 后，这时再插入一个值时，要先丢弃最早的那个值）。
			...
			if let Ok(_guard) = lock.try_lock() {
				match Self::fetch_price_n_parse() {
					Ok(price) => Self::offchain_price_with_signed_payload(price)?,
					Err(err) => return Err(err),
				}
			} else {
				return Err(<Error<T>>::AcquireStorageLockError)
			}
		...
		fn fetch_price_n_parse() -> Result<(u64, Permill), Error<T>> {
			let resp_bytes = Self::fetch_asset_from_remote().map_err(|e| {
				log::error!("fetch_asset_from_remote error: {:?}", e);
				<Error<T>>::HttpFetchingError
			})?;
			...
			let price_str = str::from_utf8(&asset_info.data.priceUsd)
				.map_err(|_| <Error<T>>::ParsePriceError)?;
			let price = parse_price_str(price_str);
			if price.is_none() {
				return Err(<Error<T>>::ParsePriceError)
			}
		...
		fn fetch_asset_from_remote() -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", HTTP_ASSET_REMOTE_REQUEST);
			...
			log::info!("timestamp: {:?}", sp_io::offchain::timestamp());
			log::info!("timeout: {:?}", timeout.clone());

			let pending = request
				.add_header("User-Agent", "curl/7.78.0")
				.deadline(timeout)
				.send()
			...
			let response = pending
				...
				.map_err(|x| {
					log::info!("1 {:?}", x);
					return <Error<T>>::HttpFetchingError
				})?
				.map_err(|x| {
					log::info!("2 {:?}", x);
					return <Error<T>>::HttpFetchingError
		...
		fn fetch_from_remote() -> Result<Vec<u8>, Error<T>> {
			...
			// two `?`   ref: https://substrate.dev/rustdocs/v2.0.0/sp_runtime/offchain/http/struct.PendingRequest.html#method.try_wait
			let response = pending
				.try_wait(timeout)
				.map_err(|_| <Error<T>>::HttpFetchingError)?
				.map_err(|_| <Error<T>>::HttpFetchingError)?;
		...
		fn offchain_price_with_signed_payload(price: (u64, Permill)) -> Result<(), Error<T>> {
			let signer = Signer::<T, T::AuthorityId>::any_account();

			log::info!("final price: {:?}", price);

			if let Some((_, res)) = signer.send_unsigned_transaction(
				|acct| Price { price, public: acct.public.clone() },
				Call::submit_dot_price_with_signed_payload,
			) {
				return res.map_err(|_| {
					log::error!("Failed in offchain_price_with_signed_payload");
					Error::<T>::OffchainUnsignedTxSignedPayloadError
				})
			}
	...
	fn parse_price_str(price_str: &str) -> Option<(u64, Permill)> {
		...
		if let Ok(integer) = price_str.parse::<u64>() {
			return Some((integer, Permill::zero()))
		}
		// case of float
		...
		if parts.len() == 2 {
			...
			return Some((parts_u, Permill::from_parts(parts_f)))
		}
		None
	}
